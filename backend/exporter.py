...
def generate_pdf_report(case_data: dict, analysis_output: str) -> bytes:
    ...
    story.append(Paragraph("NYAYAIQ CASE REPORT & AI ANALYSIS", title_style))
...
def generate_docx_report(case_data: dict, analysis_output: str) -> bytes:
    ...
    title_run = title.add_run("NYAYAIQ CASE REPORT & LEGAL ANALYSIS")
...
def generate_chat_pdf_report(chat_messages: list, username: str = "Officer", case_title: str = "General Consultation") -> bytes:
    """
    Generates a formal PDF transcript of AI assistant chat messages.
    """
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        buffer,
    ...
    story = []
    story.append(Paragraph("<b>NYAYAIQ LEGAL INTELLIGENCE - CHAT TRANSCRIPT</b>", header_style))
    story.append(Paragraph(f"Official Police Investigation Audit Record | Generated by Officer: <b>{username}</b> | Case/Subject: <b>{case_title}</b> | Date: {datetime.now().strftime('%d %b %Y %H:%M')}", sub_style))
    ...
    for msg in chat_messages:
        ...
        # Clean markdown stars for PDF printing
        content_clean = clean_formatting(content).replace('\n', '<br/>')
        
        if role == 'USER':
            story.append(Paragraph(f"<b>[OFFICER PROMPT]</b> - {timestamp}", meta_style))
            story.append(Paragraph(content_clean, user_style))
        else:
            story.append(Paragraph(f"<b>[NYAYAIQ ASSISTANT]</b> - {timestamp}", meta_style))
            story.append(Paragraph(content_clean, assistant_style))
